...
[wiki page on how to create a module](https://www.freecadweb.org/wiki/FreeCAD_Build_Tool#CreateModule) is definitely the start point!
...
The module  code is organized similar with FreeCAD source in the module folder. Gui related C++ code is located in "Gui" subfolder, whle nonGui code are put into "App" subfolder. A module folder structure with essential code for the new module can be generated by `fcbt` script
...
module source files template can be found at [src/Tools/generateTemaplates/]
...
Part module is coded in C++ for better performance, but there is one good example of pure python implemented Feature
...
How does OCC works with openInventor for 3D rendering?
